...
for ii, ID in enumerate(MANGAID):
    ...
    mastar_file_filenames.append(ra_string+dec_string+"_"+mjd_string+"_"+MANGAID_string)
...
for ii in range(mastar_file_filenames.size):
    try:
        index = np.where(PyHammerResultCondensed[:,0] == mastar_file_filenames[ii])[0][0]
...
spectypeNUM = np.empty(mathced_types_to_mastar.size, dtype=int) ## Spectral type, 0 for O to 7 for L, 8 = C, 9 = WD
specType_toNUM_NUM = np.arange(10)
specType_toNUM_alph = np.array(['O', 'B', 'A', 'F', 'G', 'K', 'M', 'L', 'C', 'WD'])
for ii in range(spectypeNUM.size):
    try:
        ...
        spectypeNUM[ii] = np.where(specType_toNUM_alph == current_mainspecType)[0][0] * 10 + current_subType
    except ValueError:
        spectypeNUM[ii] = -1
