...
# specType_order = np.array(['F', 'G', 'K', 'M', 'C', 'WD'])
# specType_order_num = np.array([4, 5, 6, 7, 8, 9])

# colors = ['blue', 'green', 'orange', 'cyan', 'indigo', 'red', 'black']
...
# ymajor_tick_space = 1
# yminor_tick_space = 0.1

# for ii, spectype  in enumerate(specType_order):
#   thisType_filenames = np.genfromtxt("SB2_IndividualSpec/"+spectype+"_list.txt", dtype="U")
#   this_color = colors[ii]
#   first = 0
#   for jj in thisType_filenames:
#       data = np.loadtxt("SB2_IndividualSpec/"+jj, delimiter=",", skiprows=1)
#       if first==0:
#           plt.plot(smoothFlux(data[:, 0]), smoothFlux(removeSdssStitchSpike(data[:, 0], data[:, 1])), color=this_color, label=spectype, linewidth=0.5, zorder=1)
#           first += 1
#       else:
#           plt.plot(smoothFlux(data[:, 0]), smoothFlux(removeSdssStitchSpike(data[:, 0], data[:, 1])), color=this_color, linewidth=0.5, zorder=1)


# plt.xlabel(r"Wavelength [$\rm{\AA}$]")
# plt.ylabel(r"Luminosity [W/$\rm{\AA}}$]")
# plt.legend(loc='best')
# ax = plt.gca()
...
# ax.yaxis.set_major_locator(ticker.MultipleLocator(ymajor_tick_space))
# ax.yaxis.set_minor_locator(ticker.MultipleLocator(yminor_tick_space))
# ax.set_yscale('log')
# plt.savefig("C_temp_plot.eps", dpi=600)
# plt.clf()
# plt.close()
...
for ii, spectype_filename in enumerate(filenames):
    ...
    data = np.loadtxt("SB2_IndividualSpec/"+spectype_filename, delimiter=",", skiprows=1)
    ...
    if first_of_spectype:
        plt.plot(smoothFlux(data[:, 0]), smoothFlux(removeSdssStitchSpike(
            data[:, 0], data[:, 1])), color=this_color, label=spectype, linewidth=1.0, zorder=1)
        first_of_spectype = False
        previous_spectype = spectype
    else:
        plt.plot(smoothFlux(data[:, 0]), smoothFlux(removeSdssStitchSpike(
            data[:, 0], data[:, 1])), color=this_color, linewidth=1.0, zorder=1)
        previous_spectype = spectype
