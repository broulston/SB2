...
cut_index = np.where((gmag_SDSSDR12 > 15.5) & (imag_SDSSDR12 > 15.5) & (M_G > 5.0))[0]
...
for ii, ID in enumerate(MANGAID):
    ...
    for jj in range(all_visits.size):
        ...
        np.savetxt(orginal_outputDIR+filename_string+".txt", spec, delimiter=",", header=header, fmt="%5.4f, %0.9e, %0.9e")
        ...
        np.savetxt(converted_outputDIR+filename_string+".txt", spec, delimiter=",", header=header, fmt="%5.4f, %0.9e, %0.9e")

np.savetxt(data_dir+"list_of_all_MaStar_specCUT.txt", all_filenames, delimiter=",", fmt="%s")
progress.done()
# plt.show()
# plt.clf()
# plt.close()